...
r = client.post('/registrations/%d/cancel' % aid, follow_redirects=True)
check('REQ-03 取消报名成功', '已取消报名' in r.get_data(as_text=True))
r = client.post('/registrations/%d/signup' % aid, follow_redirects=True)
check('REQ-03 取消后重新报名成功', '报名成功' in r.get_data(as_text=True))
